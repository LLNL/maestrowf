...
class TestLinkIntegration(unittest.TestCase):
    ...
    LINKS_0001_test_combo_var_index = {
        "VAR1.0.36/VAR2.0.17/VAR3.0.44/VAR4.0.39/test-directory-hashing":
            "test-directory-hashing/VAR1.0.3585516934.VAR2.0.1707261687.VAR3.0.4406243939.VAR4.0.3920015696",
        "VAR1.0.39/VAR2.0.75/VAR3.0.72/VAR4.0.55/test-directory-hashing":
            "test-directory-hashing/VAR1.0.3874309076.VAR2.0.7520078045.VAR3.0.718718159.VAR4.0.5491000152",
        "VAR1.0.84/VAR2.0.73/VAR3.0.90/VAR4.0.19/test-directory-hashing":
            "test-directory-hashing/VAR1.0.8368954934.VAR2.0.7296721416.VAR3.0.8958327389.VAR4.0.1895291838",
    }
    ...
    def compare_tree_to_reference(self, tree, reference):
        ...
        try:
            ...
            success = False
            for permutation in permutations(links):
                try:
                    for a, b in zip(permutation, target_list):
                        a0, a1 = a[0][-len(b[0]):], a[1][-len(b[1]):]
                        print(a0, "==", b[0])
                        print(a1, "==", b[1])
                        assert a0 == b[0]
                    ...
                    success = True
                    print(f"success debug: {permutation} / {target_list}")
            if not success:
                raise AssertionError(f"links don't match target: {links} != {target_list}")
    ...
    def test_combo_var_index(self):
        """
        test simple combo links with var variables
        """
        os.chdir(self.tmp_dir)
        integration_spec_path = self.spec_path("link_integration_fast.yml")
        ...
        maestro_cmd = ["maestro", "run", "-fg", "-y", "-s", "0", "--make-links",
               "--link-template",
               ("{{output_path}}/../links/{{date}}/{{study_name}}-{{study_index}}/"
                "combo-{{combo_index}}/VAR1.{{VAR1}}/VAR2.{{VAR2}}/"
                "VAR3.{{VAR3}}/VAR4.{{VAR4}}/{{step}}"),
               integration_spec_path]
        print(" ".join(maestro_cmd))
        tree_cmd = ["tree", "-f", "-i", os.path.join(self.tmp_dir, "output", "links")]
        ...
        tree_result = cmd_output.stdout.decode()
        print(f"tree_result:\n{tree_result}")
        self.compare_tree_to_reference(tree_result, self.LINKS_0001_test_combo_var_index)
    ...
    # def test_var_template_validatation(self):
    #     """
    #     test template error testing
    #     """
    #     os.chdir(self.tmp_dir)
    #     integration_spec_path = self.spec_path("link_integration_template_error.yml")
