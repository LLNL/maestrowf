...
def _validate_list_dictionary(sampling_dict):
    ...
    if "parameters" in sampling_dict.keys():
        ...
        for key, value in sampling_dict["parameters"].items():
            ...
            _log_assert(len(value) == sample_length, "all sample lists must be the same length")
...
def cross_product_sample(sampling_dict):
    ...
    if "parameters" in sampling_dict.keys():
        ...
        samples =  _recursive_cross_product_sample(parameters,samples=[constants])
    else:
        samples=[constants]
    LOGGER.info("samples:\n%s",str(samples))
    return _convert_dict_to_maestro_params(samples)
...
def get_custom_generator(env, **kwargs):
    ...
    try:
        SAMPLE_DICTIONARY = kwargs.get("sample_dictionary", env.find("SAMPLE_DICTIONARY").value)
    except:
        raise ValueError("this pgen code requires SAMPLE_DICTIONARY to be defined in the yaml specification")
    try:
        sample_type = SAMPLE_DICTIONARY["sample_type"]
    except:
        raise ValueError("this pgen code requires SAMPLE_DICTIONARY['sample_type'] " +
    ...
    if sample_type == "list":
        ...
        raise ValueError("The 'sample_type' of "+sample_type+" is not supported.")
