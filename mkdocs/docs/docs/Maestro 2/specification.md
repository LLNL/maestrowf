...
**Key**       |  **Required?** | **Description** |
    :-:       |   :-:          |  :-             |
[`metadata`](#metadata)        | Yes | General information about a study and its high-level purpose |
[`environment`](#environment)  | No  | Fixed constants and other values that a globally set and referenced |
[`workflow`](#workflow)        | Yes | Tasks that the study is composed of and are executed in a defined order |
[`parameters`](#parameters)    | No  | Parameters that are user varied and applied to the workflow |
...
`authors`     |      No       |    list(str)    | Names (usernames) of the user who created the study |
`maintainers` |      No       |    list(str)    | Names (usernames) of the user who maintains the study |
`project`     |      No       |    str          | Name of the project the study belongs to |
